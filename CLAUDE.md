...
- `metadata`: `name` (unique across all files), `namespace` = **the backend's namespace** (`sfg-labs` for Suwalka, `sfg-apps` for NMA/Baithak/CMS, `sfg-gateway` for Zitadel — see Namespaces above), `labels` (`sfg-project`, `sfg-auth: jwt|public|mixed`), `annotations` (`sfg-labs.in/description|team|upstream`).
  - `sfg-auth: mixed` is for a route file whose http rules genuinely split — some `openid-connect`-protected, some not — because the *backend itself* validates a second, non-Zitadel token on the unprotected rules (not because auth was just forgotten on part of the file). `routes/zimma-api.yaml` is the reference: only `POST /api/auth/sso` carries `openid-connect` (the POS→Zimma Zitadel-token handoff); every other zimma path is `cors`/`limit-req`/`request-id` only because zimma-api validates its own 7-day JWT in-app. Reach for `mixed` only when that's the actual shape — a file that's mostly `jwt` with one path accidentally left open is a bug, not a `mixed` route.
