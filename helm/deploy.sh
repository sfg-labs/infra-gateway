...
if [[ -z "${DRY_RUN}" ]]; then
  ...
  # Hard-fail, don't warn-and-continue: an unset var makes envsubst emit
  # `X-Gateway-Secret: ` with nothing after the colon, which YAML parses as
  # a NULL value, not a missing key. `kubectl apply` validates against the
  # ApisixRoute CRD's (permissive) OpenAPI schema and exits 0 — the CRD
  # itself doesn't know proxy-rewrite's plugin-level schema, which DOES
  # reject a null header value. That rejection happens downstream, when the
  # ingress controller tries to sync the CRD to the APISIX Admin API, so the
  # observable failure is the route silently failing to sync (calls to it
  # 404, "route not found") — not the clean 401 a missing secret would
  # suggest. A prior version of this guard warned-and-continued and
  # described the wrong symptom (401); both are corrected here.
